...
(function () {
  ...
  // ---- Preguntar al usuario (una vez) cuántas filas quiere por página ----
...
.pill .num{min-width:120px;text-align:center;font-weight:600}
.pill .sep{opacity:.4}
.pager-size{margin-left:10px;padding:6px 10px;border-radius:8px;border:1px solid #e5e7eb;background:#fff}
  ...
  // ---- Paginador UI (pill con « ‹ num › » y selector de filas) ----
  ...
  if (yearInput) {
    const applyYearChange = () => { currentPage = 1; currentOrder = 'asc'; triggerLoad(); };
